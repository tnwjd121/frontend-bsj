...
() => {
  ...
  a.textContent = '<h1>textContent 속성</h2>';
  b.innerHTML = '<h1>textContent 속성</h2>';
...
() => {
  ...
  rects.forEach((rect,index)=>{
    ...
    // 이미지 경로에서 너비와 높이를 직접 설정
    rect.style.width = width + 'px';
    rect.style.height = '250px'

    const src = `./cat-551554_1280.jpg`
...
// h1.sytle['backgroundColor']: 대괄호 표기법
// h1.sytle['background-color']: 대괄호 표기법

// div 25개의 스타일을 조작하여 검은색에서 흰색으로 그라데이션 효과를 보이도록 지정
document.addEventListener('DOMContentLoaded',
() => {
  // container  id 속성 아래에 있는 div 태그를 선택
  const divs = document.querySelectorAll('#container > div');
  // div 개수만큼 반복하여 출력
  divs.forEach((div, index)=> {
    const gradation = index * 10; // 0~240
    // 크기를 지정 시 단위를 반드시 같이 작성
