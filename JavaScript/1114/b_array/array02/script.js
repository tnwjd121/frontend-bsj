...
// 배열명.reduce((accumulator, currentValue, currentInedex, array) => {
  // 함수 로직
// }, initialValue);

//accumlator: 이전 리듀서의 호출의 반환 값
...
let numbers = [1, 2, 3, 4];
let sum = numbers.reduce((accumlator,
  currentValue) => accumlator + currentValue, 0);
    console.log(sum);
  ...
  // 데이터 사이에는 ', '
  // let cars = ['audi', 'bmw', 'hyundai', 'volvo'];
  let combinedCarNames = cars.reduce((accumlator,
    currentValue) => accumlator + ', ' +currentValue)
...
let firstIndexFive = numbers.findIndex(num => num > 5);
console.log(firstOverFive); // 6
console.log(firstIndexFive); //3
