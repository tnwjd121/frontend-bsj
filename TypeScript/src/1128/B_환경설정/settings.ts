...
? Node.js 설치
  ...
  : 설치 완료 여부 확인
    ...
    : npm 버전 변경
      ...
      - 특정 버전으로 변경
      npm install -g npm@9.8.0  => @뒤에 버전 번호 적으면 됨

? windows에서 npm 실행이 되지 않을 경우
  (Node.js 설치 후에도 npm 실행이 안되는 경우)

  윈도우 키 > 환경 환경 변수 편집> 시스템 속성 > 고급 > 환경 변수 클릭

  : 사용자 변수에서 Path
    '사용자 이름'에 대한 사용자 변수
    파일탐색기 > C드라이브 > Users(사용자)
    > 자신의 컴퓨터 이름(ITPS)
  ...
  !2. npm이란?
  : NPM(nodde package manager)은 Node.js의 기본 패키지 관리 도구
...
: 타입스크립트(.ts) 파일을 자바스클비트(.js) 파일로 변환하는 도구
...
: 예) 출력 디렉토리 위치, 사용할 모듈 시스템, 지원하는 ECMAScript 버전등을 관리
